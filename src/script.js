...
function cards__play_of() {
    ...
    cards_node.forEach((element) => {
        element.querySelector('.card__image').style.height = '250px';
...
function click_card_game() {
    if (event.target.parentElement.lastElementChild == collecting_sounds[0]) {
        ...
        if (collecting_sounds.length == 0) {
            ...
            if (errors == 0) {
                smile.style.backgroundImage = "url(images/burger_menu.png)";
                win_mp3.play();
            } else {
                smile.style.backgroundImage = 'url(images/failure.jpg)';
                lose_mp3.play();
            }
